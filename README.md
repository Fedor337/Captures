...
- Повторы: зонды не должны иметь длинных гомополимерных, тандемных и низкосложных повторов, так как они приводят к низкой специфичности, образованию шпилек и слабой способностью к связыванию. Отбор производился по меньше 5 гомополимерных повторов(другие типы повторов не учитывались). Запускаем код repetition_filter.py
- ΔG вторичной структуры: оценивается энергетическая стабильность нежелательных шпилек и димеров, ΔG > –6.0 ккал/моль считается допустимым. Отбор проводился по ΔG > –4.0 ккал/моль. Проверяем и выводим в фаста файл с помощью комманды:
